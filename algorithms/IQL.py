...
class IQL(object):
    ...
    def test(env, path, runner, best_score=0):
        print("start test")
        runner.start_test()
        new_path = path + 'best_'
        test_only = True
    ...
    def algorithm(map_name='3m', difficulty='3',path='model/3m_iql_CT_3/', load_model=True,
        ...
        env = StarCraft2Env(map_name=map_name, difficulty=difficulty, obs_own_health=True, obs_all_health=True,
                            ...
                            window_size_x=window_size_x, window_size_y=window_size_y, window_loc=window_loc)
        ...
        for e in range(n_episodes):
            ...
            if (e % 500) == 0:
                best_score = test(env=env, runner=runner, path=path, best_score=best_score)
...
if __name__ == '__main__':
    map_name = '3m'
    difficulty = '3'
    path = 'result/' + map_name + '_iql_CT_1/'
    main(map_name=map_name, difficulty=difficulty, load_model=True)